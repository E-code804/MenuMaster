...
const RestaurantPage = ({user, setUser}) => {
  ...
  const handleClick = async (id) => {
    const response = await fetch(`http://localhost:4000/api/restaurants/${restId}/${id}`, {
      method: 'DELETE'
    })
        ...
        <div className="pt-36 flex justify-center w-full">
          {!isLoading && (
            <div className="w-full bg-gray-200 px-6 py-12 rounded-lg shadow-lg">
              <img className="w-80 h-64 mx-auto mb-8 rounded-lg" src={restaurant.photo.url} alt="Restaurant" />
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                ...
                <div className="text-center">
                  ...
                  <p className="text-lg">
                    Cuisine: {restaurant.cuisine.map((cuisine, index) => (
                      <span key={index}>
              ...
              {restaurant.user_reviews.map((rev) => (
                <div key={rev.revId} className="bg-white p-5 rounded-lg mb-5">
                  <h1 className="text-lg font-bold">{rev.userName} - {rev.createdAt}</h1>
                  <h2 className="text-xl mt-2">Rated: {rev.ratingReview}</h2>
                  <p className="mt-4">{rev.writtenReview}</p>
                  {rev.userName === user.userName && 
                    <span className="bg-red-500 text-white cursor-pointer py-2 px-4 rounded mt-4 inline-block"
